...
class CNN3:
    ...
    def build_model(emb_matrix, max_sequence_length):
        ...
        distr_layer = TimeDistributed(Dense(300, activation='relu'))

        # Lambda layer
        lamb_layer = Lambda(lambda x: K.max(x, axis=1), output_shape=(300, ))
...
#         conv1b = BatchNormalization()(conv1b)
        ...
        merged = Dropout(0.2)(merged)
        merged = BatchNormalization()(merged)
        merged = Dense(3, activation='relu')(merged)
        merged = Dropout(0.2)(merged)
        merged = BatchNormalization()(merged)
        ...
        #ada = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
